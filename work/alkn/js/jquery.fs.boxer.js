...
if (jQuery) (function($) {
	...
	function _build(e) {
		...
		if ($("#boxer").length < 1 && (is_image || is_video || is_url || is_element || is_object)) {
			...
			html += '</div></div>'; //container, content, 
			...
			data.$photo = data.$boxer.find(".frame-photo");
			...
			$('#boxer-overlay').on('click', _close);//+Adding: 
			if (data.gallery.active) {
	...
	function _formatCaption($target) {
		var title = $target.attr("data-ja");
		var en = $target.attr("data-en");
		return (title != "" && title !== undefined) ? '<p id="frame-text-ja">' + title + '</p><p id="frame-text-en">' +en+ '</p>' : "";
	}
